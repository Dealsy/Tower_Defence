	...
	public class ForeachLoop : Node {
		...
		public override string GenerateCode() {
			...
			if(!string.IsNullOrEmpty(ta)) {
				...
				//string loopType = targetType.HasImplementInterface(typeof(System.Collections.Generic.IEnumerable<>).MakeGenericType(elementType)) ? "var " : CG.Type(elementType) + " ";
				string loopType = CG.Type(elementType) + " ";
				return CG.Flow(
