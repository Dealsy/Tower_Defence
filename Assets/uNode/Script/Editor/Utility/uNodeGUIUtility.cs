...
namespace MaxyGames.uNode.Editors {
	...
	public static class uNodeGUIUtility {
					...
					if(vType != null) {
						if(preferMemberData) {
							...
							if(IsSupportedType(vType.GetElementType()) || vType.GetElementType().IsClass) {
								...
								ShowField(new GUIContent(vName), val.GetType().GetFieldCached(nameof(ParameterValueData.value)), val, unityObject, new ObjectTypeAttribute(vType));
							}
		...
		public static string DrawSearchBar(string searchString, GUIContent label, string controlName = null, params GUILayoutOption[] options) {
			...
			searchString = EditorGUILayout.TextField(label, searchString, EditorStyles.toolbarSearchField, options);
			if(GUILayout.Button("", GUI.skin.FindStyle("ToolbarSearchCancelButton") ?? GUI.skin.FindStyle("ToolbarSeachCancelButton"))) {
				searchString = "";
