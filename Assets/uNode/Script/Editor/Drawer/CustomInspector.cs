	...
	public class CustomInspector : ScriptableObject {
			...
			if(editorData.selected is List<NodeComponent> && editorData.selectedNodes.Count > 0 && editorData.graph != null) {
				...
				for(int i = 0; i < editorData.selectedNodes.Count; i++) {
					...
					if(editorData.selectedNodes.Count == 1) {
						...
						GUI.DrawTexture(uNodeGUIUtility.GetRect(GUILayout.Width(32), GUILayout.Height(32)), uNodeEditorUtility.GetTypeIcon(selectedNode.GetNodeIcon()) ?? uNodeEditorUtility.GetTypeIcon(typeof(TypeIcons.UNodeIcon)));
		...
		public static void DrawUnitObject(UnityEngine.Object obj) {
			...
			if(obj is uNodeFunction) {
				...
				if(function.modifiers.Async) {
					if(!uNodeUtility.IsValidAsyncType(function.ReturnType())) {
						EditorGUILayout.HelpBox($@"The return type of async function must be void, Task, Task<T>, or a Type with AsyncMethodBuilder attribute", MessageType.Error);
					} else if(function.ReturnType() == typeof(void)) {
						var memberRect = EditorGUILayout.BeginVertical();
						EditorGUILayout.HelpBox(
$@"The c# support async void, but uNode support is limited to call only and cannot be used on Await node it's better to use Task.
[Click to change]", MessageType.Warning);
						EditorGUILayout.EndVertical();
						if(Event.current.button == 0 && Event.current.type == EventType.MouseDown && memberRect.Contains(Event.current.mousePosition)) {
							function.returnType = MemberData.CreateFromType(typeof(System.Threading.Tasks.Task));
						}
					}
				}
				...
				if(editor != null) {
					...
					if(system.supportAttribute) {
						...
						if(selected.AutoProperty) {
							VariableEditorUtility.DrawAttribute(selected.fieldAttributes, selected, (a) => {
								selected.fieldAttributes = a.ToArray();
							}, AttributeTargets.Field, "Field Attributes");
							VariableEditorUtility.DrawAttribute(selected.getterAttributes, selected, (a) => {
								selected.getterAttributes = a.ToArray();
							}, AttributeTargets.Method, "Getter Attributes");
							VariableEditorUtility.DrawAttribute(selected.setterAttributes, selected, (a) => {
								selected.setterAttributes = a.ToArray();
							}, AttributeTargets.Method, "Setter Attributes");
						} else {
							//if(selected.getRoot != null) {
							//	VariableEditorUtility.DrawAttribute(selected.getRoot.attributes, selected.getRoot, (a) => {
							//		selected.attributes = a.ToArray();
							//	}, AttributeTargets.Method, "Getter Attributes");
							//}
							//if(selected.setRoot != null) {
							//	VariableEditorUtility.DrawAttribute(selected.setRoot.attributes, selected.setRoot, (a) => {
							//		selected.attributes = a.ToArray();
							//	}, AttributeTargets.Method, "Setter Attributes");
							//}
						}
