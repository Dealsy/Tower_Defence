		...
		public static class TreeFunction {
			...
			public static List<TreeViewItem> CreateGraphItem(Object targetObject, MemberData reflectionValue, FilterAttribute filter) {
				...
				if(variableSystems != null && variableSystems.Count > 0) {
					if(variableSystems.Count == 1) {
						Component comp = variableSystems[0] as Component;
						if(comp) {
							var itemData = new List<GraphItem>();
							itemData.AddRange(variableSystems[0].Variables.Where(i => i != null).Select(item => new GraphItem(item, targetObject)));
							if(itemData.Count > 0) {
								itemData.Sort((x, y) => string.Compare(x.Name, y.Name, StringComparison.OrdinalIgnoreCase));
								RemoveIncorrectGraphItem(itemData, filter);
								var categTree = new SelectorCategoryTreeView("Node Variables: " + comp.gameObject.name, "", uNodeEditorUtility.GetUIDFromString("[GRAPH-GROUP]" + 0 + comp.gameObject.name), -1);
								itemData.ForEach(item => categTree.AddChild(new SelectorCustomTreeView(item, item.GetHashCode(), -1)));
								result.Add(categTree);
							}
						}
					} else {
						var tree = new SelectorCategoryTreeView("Node Variables", "", uNodeEditorUtility.GetUIDFromString("[GRAPH-GROUP-VARIABLES]"), -1);
						for(int i = 0; i < variableSystems.Count; i++) {
							Component comp = variableSystems[i] as Component;
							if(comp) {
								var itemData = new List<GraphItem>();
								itemData.AddRange(variableSystems[i].Variables.Where(v => v != null).Select(item => new GraphItem(item, targetObject)));
								if(itemData.Count > 0) {
									itemData.Sort((x, y) => string.Compare(x.Name, y.Name, StringComparison.OrdinalIgnoreCase));
									RemoveIncorrectGraphItem(itemData, filter);
									var categTree = new SelectorCategoryTreeView(comp.gameObject.name, "", uNodeEditorUtility.GetUIDFromString("[GRAPH-GROUP]" + i + comp.gameObject.name), -1);
									itemData.ForEach(item => categTree.AddChild(new SelectorCustomTreeView(item, item.GetHashCode(), -1)));
									tree.AddChild(categTree);
								}
							}
						}
						if(tree.hasChildren) 
