...
namespace MaxyGames.uNode.Nodes {
	[AddComponentMenu("")]
	public class LinkedMacroNode : CustomNode, IMacro, IVariableSystem, IRefreshable, IGeneratorInitializer {
		...
		private void Initialize(bool playmode) {
			Refresh();
			if(macroAsset != null) {
					...
					if(playmode) {
						foreach(var b in behaviors) {
							if(b is NodeComponent nodeComponent) {
								nodeComponent.OnRuntimeInitialize();
							}
						}
					} else { 
		...
		void IGeneratorInitializer.OnPreInitializer() {
			if(CG.isGenerating) {
		...
		void IGeneratorInitializer.OnPostInitializer() {
