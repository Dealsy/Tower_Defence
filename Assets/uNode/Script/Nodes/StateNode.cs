...
namespace MaxyGames.uNode {
	...
	public class StateNode : Node, ISuperNode {
		...
		public override void OnGeneratorInitialize() {
			...
			foreach(Transform t in transform) {
				var eventNode = t.GetComponent<StateEventNode>();
				if(eventNode) {
					var flows = eventNode.GetFlows();
					foreach(var flow in flows) {
						if(flow.isAssigned) {
							var targetNode = flow.GetTargetNode();
							if(targetNode != null) {
								CG.RegisterAsStateNode(targetNode);
							}
						}
					}
				}
		...
		public override string GenerateCode() {
			...
			foreach(Transform t in transform) {
				...
				if(node && CG.IsStateNode(node)) {
					onExit += CG.StopEvent(node).AddLineInFirst();
				}
			}
			var transitions = GetTransitions();
			for(int i = 0; i < transitions.Length; i++) {
			...
			foreach(Transform t in transform) {
				...
				if(comp) {
					if(comp.eventType == StateEventNode.EventType.OnEnter) {
						...
						CG.SetStateInitialization(comp, CG.Routine(CG.LambdaForEvent(CG.GenerateNode(comp))));
					}
				}
			}
