	...
	public class EventHook : CustomNode {
		...
		private string GenerateEventCodes() {
			...
			if(targetType.IsCastableTo(typeof(Delegate))) {
				...
				if(/*CG.IsRegularNode(this) &&*/ CG.CanSimplifyToLambda(body, method.ReturnType, parameterTypes)) {
					...
					if(method.ReturnType == typeof(void)) {
						...
						if(bodyNode is NodeReturn nr && nr.returnValue.GetTargetNode() is MultipurposeNode mn) {
							string result = mn.GenerateValueCode();
							if(result.Contains(")")) {
								int deep = 0;
								for(int i = result.Length - 1; i > 0; i--) {
									var c = result[i];
