...
namespace MaxyGames.uNode.Nodes {
	...
	public class MultiArithmeticNode : ValueNode, IRefreshable {
		...
		public override string GenerateValueCode() {
			...
			for(int i = 1; i < targets.Count; i++) {
				string val;
				if(targets[i].type.IsCastableTo(targetTypes[i].type)) {
					val = targets[i].CGValue();
				} else {
					val = targets[i].CGValue().CGConvert(targetTypes[i].type);
				}
				contents = CG.Arithmetic(contents, val, operatorType).Wrap();
			}
			return contents;
		}
