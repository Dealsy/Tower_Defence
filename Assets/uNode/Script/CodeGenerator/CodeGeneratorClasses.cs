...
namespace MaxyGames {
	public static partial class CG {
		...
		public class GData {
			...
			internal Dictionary<object, int> eventIDMap = new Dictionary<object, int>();
			...
			public MData AddNewGeneratedMethod(string uid, string type, params string[] parameters) {
			...
			public string GetEventID(object target) {
				...
				if(target is UnityEngine.Object || !target.GetType().IsValueType) {
					if(!generatorData.eventIDMap.TryGetValue(target, out var id)) {
						id = generatorData.eventIDMap.Count;
						generatorData.eventIDMap[target] = id;
					}
					return id.ToString();
				}
		...
		public class PData {
			...
			public string GenerateCode() {
				...
				if(autoProperty) {
					p += "{\n";
					string getter = CG.Flow(obj.getterAttributes.Select(a => TryParseAttribute(TryParseAttributeData(a))).ToArray()).AddLineInEnd() + 
						"get;".AddFirst(obj.getterModifier.GenerateCode(), !obj.getterModifier.isPublic);
					string setter = CG.Flow(obj.setterAttributes.Select(a => TryParseAttribute(TryParseAttributeData(a))).ToArray()).AddLineInEnd() + 
