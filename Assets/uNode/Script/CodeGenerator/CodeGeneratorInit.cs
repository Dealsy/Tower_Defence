	...
	public static partial class CG {
		...
		private static void Initialize() {
			if(graph != null) {
				...
				var initProcessing = graph.RootObject.GetComponentsInChildren<IGeneratorInitializer>(true);
				for(int i = 0; i < initProcessing.Length; i++) {
					initProcessing[i].OnPreInitializer();
				}
				if(stateGraph != null) {
				...
				for(int i=0;i< initProcessing.Length;i++) {
					initProcessing[i].OnPostInitializer();
				}
			}
		}
