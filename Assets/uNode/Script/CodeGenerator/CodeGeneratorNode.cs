		...
		public static string GetEvent(MemberData member) {
			...
			if(member.IsTargetingPortOrNode) {
				...
				if(member.targetType == MemberData.TargetType.FlowNode) {
					...
					return Invoke(typeof(Runtime.EventCoroutine), nameof(Runtime.EventCoroutine.Create), Value(graph), CG.Routine(LambdaForEvent(debug + GenerateNode(node))));
					...
					return Invoke(
						...
						nameof(Runtime.EventCoroutine.Create), 
						Value(graph), 
						CG.Routine(
							LambdaForEvent(debug + GenerateFlowCode(member.Invoke(null) as IFlowGenerate, node))
						)
					);
				}
			} else {
