							...
							if(list.Count > 0) {
								...
								}
								{
									...
									method.code = Flow(
										...
										Switch("Name", list, FlowInvoke("base", nameof(RuntimeBehaviour.SetVariable), "Name", "value", "op"))
									);
								}
							}
						}
				...
				for(int i = 0; i < generatorData.stateNodes.Count; i++) {
					...
					if(evt != null && evt.customExecution == null) {
						ThreadingUtil.Queue(() => {
							...
							if(yieldCount == 0 || yieldCount == 1 && strs[lastYieldIndex].Contains("yield ")) {
					...
					if(member.isStatic) {
						...
						if(debugScript && setting.debugValueNode) {
							...
							if(!generatorData.debugMemberMap.ContainsKey(member)) {
								generatorData.debugMemberMap.Add(member, new KeyValuePair<int, string>(generatorData.newDebugMapID,
