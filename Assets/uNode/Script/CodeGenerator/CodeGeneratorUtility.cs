	...
	public static partial class CG {
		...
		public static bool CanSimplifyToLambda(MemberData target, Type returnType, IList<Type> parameterTypes) {
			if(target.IsTargetingNode) {
				...
				} else if(bodyNode is NodeReturn) {
					var rNode = bodyNode as NodeReturn;
					if(rNode.returnValue.targetType == MemberData.TargetType.ValueNode) {
						var node = rNode.returnValue.GetTargetNode() as MultipurposeNode;
						if(node != null) {
